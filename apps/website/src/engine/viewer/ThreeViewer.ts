...
export class ThreeViewer {
  ...
  private frontlight = new DirectionalLight(16711680, 0.5);
  private backlight = new DirectionalLight(65280, 0.5);
  Shaders: Shader[] = [];
  needUpdate = true;
  Screen: Mesh;
  ...
  InitScene() {
    ...
    toggleGLoablLoading(true, "正在初始化场景");
    gtlfLoader.load(CDN_URL + "scene.glb", async (gltf) => {
      const group = gltf.scene;
      for (const o of group.children as Mesh[]) {
        ...
        if (texture) {
          ...
          o.material.onBeforeCompile = (shader: Shader) => {
            ...
            shader.uniforms.center = {
              value: new Vector3(1.04, 0.968, 0.35),
            };
